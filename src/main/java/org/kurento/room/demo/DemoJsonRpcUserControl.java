 ...
 */
public class DemoJsonRpcUserControl extends JsonRpcUserControlCustom {
  ...
  public DemoJsonRpcUserControl(NotificationRoomManagerCustom roomManager) {
    super(roomManager);
    System.out.println("Im DemoJsonRpcUserControl roomManager="+roomManager);
  }
  ...
  public void customRequest(Transaction transaction, Request<JsonObject> request, ParticipantRequest participantRequest) {
    ...
    try {
      ...
      if (hatOn) {
        ...
        FaceOverlayFilter faceOverlayFilter = new FaceOverlayFilter.Builder(roomManager.getPipeline(pid)).build();

        faceOverlayFilter.setOverlayedImage(this.hatUrl, this.offsetXPercent, this.offsetYPercent, this.widthPercent, this.heightPercent);
        roomManager.addMediaElement(pid, faceOverlayFilter);
        transaction.getSession().getAttributes().put(SESSION_ATTRIBUTE_HAT_FILTER, faceOverlayFilter);
      }
      else
        {
        if (!transaction.getSession().getAttributes().containsKey(SESSION_ATTRIBUTE_HAT_FILTER)) {
        ...
        roomManager.removeMediaElement(pid, (MediaElement) transaction.getSession().getAttributes().get(SESSION_ATTRIBUTE_HAT_FILTER));
  ...
  public void joinRoom(Transaction transaction, Request<JsonObject> request, ParticipantRequest participantRequest) throws IOException, InterruptedException, ExecutionException {
    ...
    log.info("!! Join room request. Room name="+roomName+"  roomManager="+roomManager);
    //int totalParticipants = getRoomParticipantsQuantity(roomName);

    NotificationRoomManagerCustom manager = (NotificationRoomManagerCustom)roomManager;
    int totalParticipants = manager.totalParticipants(roomName);

    log.info("Room '"+roomName+"'  total participants="+totalParticipants);
